...
# 假设config包含必要的配置参数
class Config:
    # train_data_file = r'/home/s2024244178/data/cognitive/train_simp_cleaned_0708_13.jsonl'
    # test_data_file = r'/home/s2024244178/data/cognitive/test_simp_cleaned_0708_13.jsonl'
    # val_data_file = r'/home/s2024244178/data/cognitive/val_simp_cleaned_0708_13.jsonl'

    # train_data_file = r'/home/s2024244178/data/cognitive/train_simp_cleaned_0708_13+5neg-summary.jsonl'
    # val_data_file = r'/home/s2024244178/data/cognitive/val_simp_cleaned_0708_13+5neg-summary.jsonl'
    # test_data_file = r'/home/jd/code/dyq/TELLER/data/cognitive/test_simp_cleaned_0708_13+neg.jsonl'

    # val_data_file = '/home/jd/code/dyq/TELLER/data/cognitive/val_simp_cleaned_0708_13+0.2neg-summary.jsonl'
    # test_data_file = '/home/jd/code/dyq/TELLER/data/cognitive/test_simp_cleaned_0708_13.jsonl'
    # train_data_file = '/home/jd/code/dyq/TELLER/data/cognitive/train_simp_cleaned_0708_13+0.2neg-summary_chunk1.jsonl'

    # train_data_file = r'/home/jd/code/dyq/TELLER/data/cognitive/train_simp_cleaned_0708_13+5neg-summary_remain4.jsonl'
    # train_data_file = r'/home/jd/code/dyq/TELLER/data/cognitive/train_simp_cleaned_0708_13+5neg-summary.jsonl'
    # test_data_file = '/home/jd/code/dyq/TELLER/data/cognitive/test_simp_cleaned_0708_13+5neg-summary.jsonl'
    # val_data_file = '/home/jd/code/dyq/TELLER/data/cognitive/val_simp_cleaned_0708_13+5neg-summary.jsonl'

    train_data_file = '/home/jd/code/dyq/TELLER/data/cognitive/train_simp_cleaned_0708_13+0.4neg-summary_chunk1.jsonl'
    # val_data_file = '/home/jd/code/dyq/TELLER/data/cognitive/val_simp_cleaned_0708_13+0.4neg-summary.jsonl'
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    # save_dir = './HGT_saved_models_ce_dynamic_evidence_seed42_tuning_13label+5neg-summary_simplified'

    # save_dir = './HGT_saved_models_ce_dynamic_evidence_seed42_tuning_13label+0.2neg-summary_simplified'

    save_dir = './HGT_saved_models_ce_dynamic_evidence_seed42_tuning_13label+0.4neg-summary_simplified'
    batch_size = 32
...
tokenizer = BertTokenizer.from_pretrained('/home/jd/code/wyj/Trust_TELLER-main/chinese-roberta-wwm-ext')
bert_model = RobertaModel.from_pretrained('/home/jd/code/wyj/Trust_TELLER-main/chinese-roberta-wwm-ext').eval()
...
def process_graph_data(filename):
    ...
    graph_list = []
    # bias_mapping = {
    #     # 要求去掉 7虚幻真相 和 9单纯曝光效应
    #     "确认偏差": 0, "可用性启发式": 1, "刻板印象": 2,
    #     "光环效应": 3, "权威偏见": 4, "框架效应": 5, "从众效应": 6,
    #     # "虚幻真相效应": 7,
    #     "群体内偏爱": 8,
    #     # "单纯曝光效应": 9,
    #     "对比效应": 10, "过度自信效应": 11, "损失厌恶": 12,
    #     "结果偏差": 13, "后见之明偏差": 14
    # }
    ...
    for line in tqdm(lines):
        ...
        if data_id > 100000000:
            ...
            prev_sentences = [data['evidence'][0]]
            next_sentences = []
...
class HGTModel(nn.Module):
    def __init__(self, predicate_bert_embs, hidden_size, num_heads, num_layers, metadata):
        ...
        # 分类器（带初始化）
        # 加上负样本，变成14分类
        self.classifier = Linear(hidden_size, 14)
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        for layer in self.convs:
            ...
            print("residual_dict keys:", residual_dict.keys())
            # 归一化 + 残差 + 激活 old
            ...
            x_dict = {
                k: F.elu(norm_dict[k](x_dict[k] + residual_dict[k]))
                for k in x_dict if k in norm_dict and k in residual_dict
            }
...
def train_model(model, train_loader, val_loader):
    ...
    # print(f'Test Accuracy: {test_acc:.4f}, {test_loss:.4f}')
    ...
    for epoch in range(config.epoch):
        ...
        for batch in tqdm(train_loader):
            ...
            optimizer.zero_grad()
            pred, _ = model(batch.x_dict, batch.edge_index_dict)
...
def extract_news_attention_by_index(
    alpha: torch.Tensor,
    batch_size: int,
    num_nodes_per_graph: int = 107,
    num_predicates: int = 106
) -> torch.Tensor:
    """
    直接根据编号提取每个图中 news 节点（编号 i*107）对 106 个 predicate 节点（编号 i*107 + 1 到 i*107 + 106）的注意力。
    要求 alpha 已经是 mean over heads 的 shape: [num_edges]
    且顺序与 edge_index 保持一致（如 news<-pred）。

    返回:
        batch_attention: [batch_size, num_predicates]
    """
    # 1. 对多头 attention 取平均: [num_edges, num_heads] -> [num_edges]
    alpha = alpha.mean(dim=1)  # [E]
    # print(f"[Info] alpha shape: {alpha.shape}")  # [num_edges]

    batch_attention = torch.zeros(batch_size, num_predicates)

    for batch_id in range(batch_size):
        news_node_idx = batch_id * num_nodes_per_graph
        pred_indices = torch.arange(1, num_predicates + 1) + batch_id * num_nodes_per_graph  # [1~106 + offset]

        # 对应边在 alpha 中的索引（顺序假设和 edge_index 一致）
        # 假设 alpha 中顺序就是：pred -> news
        for local_pid, global_pid in enumerate(pred_indices):
            edge_idx = batch_id * num_predicates + local_pid
            if edge_idx < alpha.size(0):
                batch_attention[batch_id, local_pid] = alpha[edge_idx].item()
                # if batch_id < 2 and local_pid < 3:  # 打印部分调试信息
                #     print(f"[Match] Batch {batch_id}, Pred {local_pid}, Edge {edge_idx}, Attention: {alpha[edge_idx]:.4f}")
            else:
                print(f"[Warning] Edge index {edge_idx} out of bounds for alpha of shape {alpha.shape}")

    return batch_attention


# 每个图含 1 + 106 = 107 个节点。
# batch_size = 32，总节点数 32 × 107 = 3424
# 注意力边：('predicate', 'to_news', 'news') 中，谓词 → 新闻
# 返回的 alpha shape 是 [362944, 4]，表示：
# 总边数是 362944
# 多头注意力 num_heads = 4
...
def predict(model, loader, att_file):
    ...
    with torch.no_grad():
        for batch in tqdm(loader):
            ...
            batch = batch.to(config.device)
            pred, att = model(batch.x_dict, batch.edge_index_dict)
            correct += (pred.argmax(1) == batch.y.squeeze()).sum().item()
            print("pred.argmax(1):", pred.argmax(1))
            print("batch.y.squeeze():", batch.y.squeeze())
            ...
            attention_probs = extract_news_attention_by_index(
                alpha=att,
                batch_size=32
            )
...
def main():
    ...
    dummy_sample = train_dataset.get(0)  # 获取一个样本
    ...
    # 预测并保存attention
    # predict(model, val_loader, 'hgt_val_attentions_dynamic_evidence_tuning_13label+0.4neg-summary_simplified_temp0.5.jsonl')
    predict(model, train_loader, 'hgt_train_attentions_dynamic_evidence_tuning_13label+0.4neg-summary_simplified_chunk1_temp0.5.jsonl')
    # predict(model, test_loader, 'hgt_test-13label+neg_attentions_dynamic_evidence_tuning_13label+0.4neg-summary_simplified_temp0.5.jsonl')
