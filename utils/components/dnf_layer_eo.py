...
# tokenizer = AutoTokenizer.from_pretrained("/home/sdd/jd/dyq/Pre_model/chinese-roberta-wwm-ext")
...
FlAG_S = False  # 是否加前面的s binary 分label 训练
Label_Mapping_Rule = {"binary": {'1': 1, '0': 0},
...
def transform_org_to_logic(configure, set, gq, mask_flag=-2):
    ...
    for sample in set:
        ...
        if str(sample["ID"]) in gq_keys:
            ...
            text_input.append(sample['MESSAGE']) # 这里直接原文本送入吧
            # evidence_input.append(sample['EVIDENCE']) # for context HGT
...
def batch_generation(text_input, logics_input, label_input, mode, batchsize):
    assert len(logics_input) == len(label_input), "produce error when generate data splits"

    # Tokenize text_input and convert to tensor

    tokenizer = AutoTokenizer.from_pretrained("/home/s2022245019/model/chinese-roberta-wwm-ext")
    text_tokenized = [tokenizer(text, padding='max_length', truncation=True, max_length=30, return_tensors="pt") for
                      text in text_input]
    ...
    print("====text_ids=====", len(text_ids)) # 311
    text_ids = [torch.stack(text_ids[i:i + batchsize]) for i in range(0, len(text_ids), batchsize)]
    # 将 text_ids 填充成相同的长度（这里默认填充的方式是 'long' 类型的 0 填充）
    attention_masks = [torch.stack(attention_masks[i:i + batchsize]) for i in range(0, len(attention_masks), batchsize)]

    print("====new_text_ids=====", len(text_ids)) # 5
    ...
    # text_ids = [torch.tensor(text_ids[i:i + batchsize]) for i in range(0, len(text_ids), batchsize)]

    return [(text_ids[i], attention_masks[i], logics_input[i], label_input[i]) for i in range(len(logics_input))]


def get_min_non_zero_abs_weight(abs_weight):
    non_zero_abs_weight = abs_weight[abs_weight != 0]
    if non_zero_abs_weight.numel() > 0:
        return torch.min(non_zero_abs_weight, dim=1)[0]
    else:
...
class SemiSymbolic(nn.Module):
    def __init__(
        ...
        #     正态分布/高斯分布，
        # 大约 68.27% 的 self.weights 值会处于区间
        # [−0.1,0.1]
        #  内。
        # 大约 95.45% 的 self.weights 值会处于区间
        # [−0.2,0.2]
        #  内。
        # 大约 99.73% 的 self.weights 值会处于区间
        # [−0.3,0.3] 内。
        else:
        ...
        # 改这里
        self.logic_type = "min"
        # self.logic_type = "min" # 除0外的min ce_loss contains NaN or Inf values
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        # torch.Size([128, 90, 106])
        #
        # print("====input.unsqueeze(-1),size",
        #       input.unsqueeze(-1).size())
        # torch.Size([128, 106, 1])

        abs_weight = torch.abs(self.weights.expand(input.size(0), -1, -1) * input.unsqueeze(-1))
        # print("====abs_weight,size", abs_weight.size())
        ...
        if self.logic_type == "min":
            # print("min")
            # max_w = torch.max(abs_weight, dim=1)[0]
            # print("=====max_w ", max_w)

            nonzero_weight = torch.where(
                abs_weight > self.epsilon, abs_weight, 100.0
            )
            nonzero_min = torch.min(nonzero_weight, dim=1)[0]
            # print("=====nonzero_min",nonzero_min)

            max_abs_w = nonzero_min
...
class Conjunction_Shuffle(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        #  torch.Size([bs, 106, coj-num])。  * Input: N x P e([bs, 106, 1])
...
class Conjunction(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        # RuntimeError: The size of tensor a (106) must match the size of tensor b (15) at non-singleton dimension 1
...
class ConjunctionEO(nn.Module):
    def __init__(
        ...
        # configure: {]
        super(ConjunctionEO, self).__init__()
        self.configure = configure
        self.in_features = out_features # P 的第二维（实例化个数 1+3+1）求和，作为输入特征维度
        self.layer_type = layer_type
        # generate input features and weights by configure
        self.out_features = out_features  # Q

        self.weights = nn.Parameter(
            torch.empty((self.out_features, 1))
        )
        if weight_init_type == "normal":
            nn.init.normal_(self.weights, mean=0.0, std=0.1)
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        mask = torch.where(input >= -1, torch.tensor(1, device=input.device),
                           torch.tensor(0, device=input.device)).unsqueeze(-1).repeat(1, 1, self.out_features)
        # abs_weight: N x P x Q
        # print("===== weights.shape", weights.shape)      # torch.Size([15, 15])
        #
        # print("===== weights.expand(input.size(0), -1, -1).shape",  weights.expand(input.size(0), -1, -1).shape)
        # #  torch.Size([128, 15, 15])
        #
        # print("===== input.unsqueeze(-1).shape",  input.unsqueeze(-1).shape)
        # # torch.Size([128, 15, 1])


        abs_weight = torch.abs(weights.expand(input.size(0), -1, -1) * input.unsqueeze(-1))
        # * 逐元素（element-wise）相乘，也叫 Hadamard 积。
        #  Input: N x P   input.unsqueeze(-1)) N x P 1。    wei N Q x P
        # RuntimeError: The size of tensor a (106) must match the size of tensor b (15) at non-singleton dimension 1
...
class HGT_DNF(nn.Module):
    ...
    def forward(self, text, evidence, mask, input: Tensor) -> tuple[Any, None]:
        ...
        if self.graph_flag:
            ...
            data = process_graph_data(text, evidence)
            ...
            if self.graph_merge == "multiply":
                ...
                inner = tanh_adjustment(input, attention_to_node_0_mean)  # s是scale 后的，attention_to_node_0_mean是原始的
            else:
...
class GNN_DNF(nn.Module):
    ...
    def forward(self, text, mask, input: Tensor) -> tuple[Any, None]:
        ...
        if self.graph_flag:
            ...
            text_embeddings = self.roberta(input_ids=text, attention_mask=mask).last_hidden_state.mean(dim=1).to(text.device) # 平均pooling shape: [batchsize, 768]

            # print("=====text_embeddings.shape=====", text_embeddings.shape) #  torch.Size([32, 1024])

            predicate_embeddings = predicate_tensors.to(text.device) # ([N, 1024])目前是8  # 编码好的谓词向量 写在文件里，直接读取吧，放在代码最前面读取，这里直接赋值

            # 构建全连接图的节点初始向量
            node_embeddings = torch.cat(
                [text_embeddings.unsqueeze(1), predicate_embeddings.unsqueeze(0).expand(batchsize, -1, -1)], dim=1).to(text.device)

            # 打印节点特征的形状
            # print("=====node_embeddings.shape=====", node_embeddings.shape)  # torch.Size([32, 107, 1024]) torch.Size([64, 9, 1024]) [batchsize, num_nodes, 768]

            # 构造全连接的邻接矩阵（包括text和predicate之间的边以及predicate和predicate之间的边）
            num_nodes = node_embeddings.shape[1]  # [batchsize, num_nodes, 768] => num_nodes
            ...
            # 初始化GAT网络层
            gat_model = GraphAttentionNetwork(in_channels=node_embeddings.shape[2], hidden_channels = 256, out_channels=64, heads=1).to(text.device) # in 1024,hi, out 64
            ...
            for batch in loader:
                ...
                # print("====GAT embedding size", out.size()) #=GAT embedding size torch.Size([32, 107, 64])
                # print("====GAT embedding",out)
            ...
            if self.graph_merge == "multiply":
                ...
                inner = tanh_adjustment(input, attention_to_node_0_mean)  # s是scale 后的，attention_to_node_0_mean是原始的
                inner = torch.from_numpy(inner).float().to(input.device)
            else:
                print("\n====wrong name of graph_merge======")
                exit()
...
class GNN_DNFEO(GNN_DNF):
    ...
    def __init__(
        ...
        super().__init__(
            num_preds, num_conjuncts, n_out, delta, configure,weight_init_type,graph_flag,shuffle,graph_merge, roberta_model_path
        )
        ...
        self.eo_constraint = ConjunctionEO(
            configure=configure,  # P
            out_features=n_out,  # Q
            layer_type=SemiSymbolicLayerType.CONJUNCTION,
            delta=delta,
        )  # weight: R x R
    ...
    def forward(self, text, mask, input: Tensor,mode) -> Tensor:

        disj, _ = super().forward(text, mask, input)
...
class LogicTrainer:
    def __init__(self, num_conjuncts, n_out, delta, configure, weight_init_type, device, args, exp=None):
        ...
        if args.type_of_logic_model == "logic": # no 通过GNN + garph false 实现
            self.logic_model = DNF(nnum_preds=106, um_conjuncts=num_conjuncts, n_out=n_out, delta=delta, configure=configure,  # args.initial_delta= 0.01
                                   weight_init_type=weight_init_type).to(device)
        elif args.type_of_logic_model == "logic_eo": # no
            self.logic_model = NeuralDNFEO(num_preds=106, num_conjuncts=num_conjuncts, n_out=n_out, delta=delta,
                                   configure=configure,
                                   weight_init_type=weight_init_type).to(device)
            ...
            self.logic_model = GNN_DNF(num_preds = 106, num_conjuncts=num_conjuncts, n_out=n_out, delta=delta, configure=configure,
                                   weight_init_type=weight_init_type, graph_flag = args.graph_flag, graph_merge = args.graph_merge).to(device)
        elif args.type_of_logic_model == "gnn_logic_eo":
            self.logic_model = GNN_DNFEO(num_preds = 106, num_conjuncts=num_conjuncts, n_out=n_out, delta=delta, configure=configure,
                                   weight_init_type=weight_init_type, graph_flag = args.graph_flag, graph_merge = args.graph_merge).to(device)
        ...
        if args.focal_alpha == "alpha1": # 差异最小
            self.alpha = [1.1135856097836927, 1.1655539811342945, 1.2189099842720732, 1.2498730000264116, 1.2533898762916564, 1.2797705794659595, 1.3006006695679493, 1.31009460877715, 1.4326544099239886, 1.4388577414488666, 1.5550319074664687, 1.5713080210486097, 1.6002287078920014, 1.6323716204658443, 1.6834580875735297]
        elif args.focal_alpha == "alpha2":
            self.alpha = [1.8807908085824854, 2.410071809351543, 3.0411733067499815, 3.449089645829311, 3.4973775009972528, 3.8723688846763205, 4.184427390053088, 4.331339085825812, 6.490084447075177, 6.612173059787246, 9.119998141410997, 9.504093898414279, 10.205777517506547, 11.013922293889948, 12.35783957573458]
        elif args.focal_alpha == "alpha3":
            self.alpha = [1.0, 1.6420220249357202, 2.614576493762311, 3.363008594565055, 3.4578331715788937, 4.239088233452314, 4.949839134249781, 5.303509871513632, 11.907475813544416, 12.359685959466862, 23.513025356026397, 25.53526970954357, 29.444976076555022, 34.292806484295845, 43.172193877551024]
        elif args.focal_alpha == "alpha4": # 差异最大
            self.alpha = [3.537374065648359, 5.808446126231019, 9.248735081688618, 11.896219384966964, 12.23164938448179, 14.99524077900933, 17.5094325826265, 18.76049827640238, 42.12119613016711, 43.72083257257623, 83.17436609934005, 90.32780082987551, 104.15789473684211, 121.306484295846, 152.71619897959184]
        else:
            print("Wrong alpha")
            exit()
    ...
    def train(self, train_set, validloader, testloader):
        ...
        dir_save = "lr" + str(self.args.lr) + "_decay" + str(self.args.weight_decay) + "_numcoj"+ str(self.args.num_conjuncts) + "_epoch_"+ str(self.args.n_epoch) + "_bs_" + str(self.args.batchsize) + "_gflag_"+ str(self.args.graph_flag) +"_gmerge_"+ self.args.graph_merge + "_loss_" + self.args.type_of_loss + "_alp_" + str(self.args.focal_alpha)
        ...
        for epoch in range(self.args.n_epoch):
            ...
            trainloader = batch_generation(train_text_inputs, train_logics_inputs, train_label_inputs, self.args.mode, self.args.batchsize)
            ...
            for batch in trainloader:
                ...
                gt.append(targets)
                texts, masks, inputs, targets = texts.to(self.device), masks.to(self.device), inputs.to(self.device), targets.to(self.device) # 这里inputs 是
                ...
                outputs, saved_variable = self.logic_model(texts, masks, inputs, "train")  #texts,是文本转成id, inputs是谓词分数，调用DNF
            ...
            test_acc, test_macro_f1, test_macro_precision, test_macro_recall,  test_acc_expdum, test_macro_f1_expdum, test_macro_precision_expdum, test_macro_recall_expdum = self.test(epoch, self.logic_model,
                                                                                         testloader)
        ...
        print("\nBest Test Acc: {:.5f}".format(self.best_test_metric))
    ...
    def validate(self, epoch, net, validloader):
        ...
        with torch.no_grad():
            for batch in validloader:
                texts, masks, inputs, targets = batch[0], batch[1], batch[2], batch[3]
                gt.append(targets)
                texts, masks, inputs, targets = texts.to(self.device), masks.to(self.device), inputs.to(self.device), targets.to(self.device)
                outputs, saved_variable = self.logic_model(texts, masks, inputs, "train")
    ...
    def test(self, epoch, net, testloader):  # old
        ...
        with torch.no_grad():
            for batch in testloader:
                ...
                gt.append(targets)
                texts, masks, inputs, targets = texts.to(self.device), masks.to(self.device), inputs.to(self.device), targets.to(self.device)
                outputs, saved_variable = self.logic_model(texts, masks, inputs, "test")

                outputs_log = nn.Tanh()(outputs)
                # 判断每一行的所有元素是否都小于0
                mask = (outputs_log < 0).all(dim=1)  # shape: (batch_size,)
                for i, flag in enumerate(mask):
                    if flag:
                        print(f"[{i}] All dims < 0 → text: {texts[i]}, {outputs_log[i]}")

                # outputs, saved_variable = self.logic_model_test(texts, masks, inputs)
                loss = self.criterion(outputs, targets).item() + loss
                # inter outputs from outputs of self.logic_model
                pt.append(self.obtain_label(outputs.cpu()))
            gt = torch.cat(gt).tolist()
        ...
        print(
            "\nTest: Loss {:.5f}       Acc:{:.5f}       F1:{:.5f}    Precision:{:.5f}     Recall:{:.5f}".format(loss, acc,
                                                                                                              ...
                                                                                                              macro_precision,
                                                                                                              macro_recall))
        return acc, macro_f1, macro_precision, macro_recall, acc_expdum, macro_f1_expdum, macro_precision_expdum, macro_recall_expdum
