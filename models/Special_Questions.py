...
SQ_PROMPTS = {'STATEMENTS': """To verify the MESSAGE, what are the critical claims related to this message we need to 
verify? Please use the following format to answer. If there is no important claims, answer “not applicable”.
...
class SpecialQuestion:
    ...
    def run(self, batch_size=10):
        ...
        for chunk in tqdm(dataset_chunks):
            # construct input
            # 让大模型划分statement的promt，通过每个数据对promt模板实例化
            full_prompts = self.sq_prompt_generation(chunk)
            try:
                # 批次调用大模型，划分数据集的statement
                ...
                for sample, full_prompt in zip(chunk, full_prompts):
                    try: # 这里运行出错了，模型调用失败？
                        ...
                        # print("=============openai_api模型调用成功==============")
                        self.update_results(sample, output)
                    except:
                        # print("=============openai_api模型调用失败==============")
        ...
        print("The results of this experiment has been saved to {} for special question {}".format(self.save_path,
                                                                                                   args.sq))
    ...
    # 样本的 id 未能正确生成 STATEMENTS，为其分配一个空列表，避免后续访问时报错
    def fix_error(self, sample):
        # print("============样本的 id 未能正确生成 STATEMENTS，为其分配一个空列表，避免后续访问时报错!!!========")
    ...
    def sq_prompt_generation(self, chunk):
        ...
        if self.args.sq == 'STATEMENTS':
            if self.dataset_name == 'LIAR-PLUS':
                ...
                full_prompts = [SQ_PROMPTS[self.args.sq].replace("$MESSAGE$", self.encoding.decode(
                    self.encoding.encode(sample['message'].strip())[:1000]))
                                for sample in chunk]
        else:
            # for
            full_prompts = None
...
def parse_args():
    ...
    parser.add_argument('--save_all_path', default='../data/report1.jsonl', type=str)
...
if __name__ == "__main__":
    ...
    algorithm = SpecialQuestion(args)
    algorithm.run()
