...
from utils.data_reading import read_jsonl_file, random_sampling, read_openapi, seed_everything, label_re_mapping, \
    write_to_jsonl, extra_info, get_formatted_time
from utils.openaiLLM import OpenAIModel
from utils.evaluation import acc_compute, calculate_macro_f1
import string

"""
This class for the standard prediction method for all datasets. 
Include binary and multiple classification Setting. 
Include evaluation.

使用 OpenAI 的 API（如 GPT-3.5-turbo 或 text-davinci-003）对数据集进行文本分类，并对预测结果进行评估。
该分类任务包含 二分类（binary） 和 多分类（multiple） 两种模式
"""

# 定义了 binary（二分类）和 multiple（多分类）两种任务的提示词
# K: "External Info: " 代表是否提供额外信息（如知识库支持）
HYPER_PROMPTS = {'LIAR-PLUS-binary': {
    'Prompt': 'You will be provided with a statement, and your task is to classify its truthfulness as true or false. \n',
    "Q": "Statement: ",
    "K": "External Info: ",
    "A": "Answer: "
    },
                 'LIAR-PLUS-multiple': {
                     'Prompt': 'You will be provided with a statement, and your task is to classify its truthfulness into one of six categories. The six categories are half true, false, mostly true, barely true, true and pants fire.\n',
                     "Q": "Statement: ",
                     "K": "External Info: ",
                     "A": "Answer: "},
                 }
...
# 返回构造的promt
# 函数中 examples 是 few-shot learning 参考示例，sample 是当前需要预测的样本
def construct_prompt(examples: list, sample, hyper_parameters: dict, data_name, know):
    # with \n as suffix
    prompt = hyper_parameters['Prompt']
    for example in examples:
        prompt += (hyper_parameters["Q"] + example["statement"].strip() + "\n")
        if args.know is not None:
            prompt += (hyper_parameters["K"] + extra_info(example, data_name, know) + "\n")
...
# 从训练集中选择 shot_number 个示例，每个类别选 shot_number 个，返回examples
...
class StardPrediction:
    def __init__(self, args):
        ...
        # 创建 OpenAIModel 实例，用于访问 OpenAI API
        self.openai_api = OpenAIModel(args.api_key, args.model_name, args.stop_words, args.max_new_tokens,
                                      args.logprobs, args.echo)
        # LABEL_DEFINITION字典，存储数据集和模式对应的标签映射
        self.labels = LABEL_DEFINITION['-'.join([self.dataset_name, self.mode])]
        print("Standard Predication Baseline\n")
        print(args)
        ## data loading
        # print("Data Loading\n")
        # 加载test数据、用于few-shot的train数据、映射标签等等
    ...
    def run(self, batch_size=10):
        # 如果 num_eval_samples < 0，则使用整个数据集，否则随机采样 num_eval_samples 个样本
        ...
        # 将数据集拆分为小批次，每个批次 batch_size 个样本
        ...
        for iteration in range(self.run_number):
            ...
            for chunk in tqdm(dataset_chunks):
                full_prompts = [construct_prompt(examples, sample,
                                                 HYPER_PROMPTS['-'.join([self.dataset_name, self.mode])],
                                                 self.dataset_name, self.args.know) for sample
                                in chunk]
                try:
            ...
            print("Interation: {}, Macro_precision: {:.6f}, Macro_recall: {:.6f}".format(iteration, macro_precision,
                                                                                         macro_recall))
            f_acc = acc + f_acc
        ...
        save_path = os.path.join(self.save_path, '-'.join(
            [self.mode, str(self.shot_number), str(self.num_eval_samples), str(self.model_name), time]) + '.jsonl')
    ...
    def update_results(self, sample, response):
        if self.model_name == 'text-davinci-003':
            self.result_dict[sample['statement id']]['pt'].append(
                response['choices'][0]['text'].strip().rstrip(string.punctuation).lower())
            self.result_dict[sample['statement id']]['response'].append(response)
        elif self.model_name == 'gpt-3.5-turbo':
            self.result_dict[sample['statement id']]['pt'].append(
                response['choices'][0]['message']['content'].strip().rstrip(string.punctuation).lower())
            self.result_dict[sample['statement id']]['response'].append(response)
...
def parse_args():
    ...
    parser.add_argument('--model_name', type=str, default='text-davinci-003',
                        choices=['text-davinci-003', 'gpt-3.5-turbo'])
